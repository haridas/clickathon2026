"""CLI for hourly rollups and out-of-sample Prophet forecast runs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Hourly InMobi RCA baseline pipeline.")
    ...
    parser.add_argument("--days", type=int, default=35, help="Prior completed UTC days used to train each hourly model (minimum 21).")
    parser.add_argument("--target-day", type=parse_date, help="Completed UTC day to forecast and score, YYYY-MM-DD.")
    materialize = parser.add_mutually_exclusive_group()
    materialize.add_argument("--materialize-day", type=parse_date, help="Insert one finalized raw-event day into metrics_hourly, then exit.")
    materialize.add_argument("--backfill-start", type=parse_date, help="First UTC day (inclusive) to backfill into metrics_hourly; requires --backfill-end.")
...
def main() -> None:
    ...
    if args.materialize_day:
        ClickHouseClient().materialize_hourly_rollup(args.materialize_day)
        return
    if args.backfill_start or args.backfill_end:
        ...
        while current < args.backfill_end:
            client.materialize_hourly_rollup(current)
            current += timedelta(days=1)
        return
    if args.days < 21:
        raise ValueError("--days must be at least 21 for three weekly hourly cycles.")
    dimensions = tuple(dict.fromkeys(args.dimension or SUPPORTED_DIMENSIONS))
    target_day = args.target_day or (datetime.now(timezone.utc).date() - timedelta(days=1))
    baselines = AnomalyPipeline().run(DetectionRequest(args.metric, dimensions, target_day, args.days, not args.dry_run))
    if args.dry_run:
